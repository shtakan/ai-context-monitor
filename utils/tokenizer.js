...
const Tokenizer = {
  ...
  countTokens(text) {
    ...
    while (i < text.length) {
      ...
      if (type === 'cyrillic') {
        // Кириллица: делитель 1.8 (реальный коэффициент для русского ≈1.8 симв/токен).
        // O-23 (F5): делитель 4.0 приравнивал кириллицу к латинице и давал систематическую
        // НЕДООЦЕНКУ русского текста — измеренные пары O-23 (Δ=tokens~−serverTokens):
        // −67 при serverTokens=78 и −18172 при serverTokens=105217; пины и арифметика
        // фикстур — tests/o23-tokens-calibration.test.js.
        // Граница: делитель 4.0 опирался на один живой BYOK-прогон (216881 симв →
        // 63713 ток.); решение владельца O-23 — коэффициент 1.8. Пины, кодировавшие
        // прежнее значение, перекалиброваны в tests/tokenizer-cyrillic-calibration.test.js.
        let cyrillicCount = 0;
