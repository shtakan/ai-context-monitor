...
// v1.8.1: синтетическая анонимная min-fixture (tests/fixtures/gemini-head-sample.min.txt,
// ~2.7 КБ, тот же batchexecute/hNvQHb формат) — тест больше не скипается.
describe('Gemini живой путь на min-fixture (gemini-head-sample.min.txt)', () => {
  const fs_min = require('fs');
  const path_min = require('path');
  const logic_min = require('../../utils/gemini-intercept-logic');
  const MIN_FIXTURE = path_min.join(__dirname, '..', 'fixtures', 'gemini-head-sample.min.txt');

  function loadMin() {
    const raw = fs_min.readFileSync(MIN_FIXTURE, 'utf8');
    const r = parseGeminiHistory(raw);
    const orderItems = r.messages.map((m, i) => ({
      id: m.id,
      turnId: String(m.id).replace(/_(user|assistant)$/, ''),
      r1: m.r1 || null,
      order: i,
      role: m.role,
      text: m.text
  ...
  it('min-fixture существует и разбирается: 4 сообщения (2 хода), модель распознана', () => {
    expect(fs_min.existsSync(MIN_FIXTURE)).toBe(true);
    const { r } = loadMin();
    expect(r.count).toBe(4);
    expect(r.model).toBe('Gemini 2.5 Pro');
  });

  it('голова цепочки = user «Ты — синтетический тестовый вопрос…» → assistant «**Синтетический ответ один**», инверсий нет', () => {
    const { r, orderItems } = loadMin();
    const chain = logic_min.orderByR1Chain(orderItems);
    expect(chain.ok).toBe(true);
    const byId = {};
    for (const it of orderItems) byId[it.id] = it;
    const ids = chain.ids;
    expect(logic_min.countR1Inversions(ids.map(id => byId[id]))).toBe(0);
    // голова: user + assistant головного хода, user первым
    expect(byId[ids[0]].role).toBe('user');
    expect(byId[ids[1]].role).toBe('assistant');
    expect(byId[ids[0]].text).toContain('Ты — синтетический тестовый вопрос');
    expect(byId[ids[1]].text).toContain('**Синтетический ответ один**');
  });

  it('user-блок головного хода не теряется и стоит первым в финальном порядке', () => {
    const { orderItems } = loadMin();
    expect(orderItems.findIndex(m => /синтетический тестовый вопрос/i.test(m.text))).toBe(0);
  });
});


const fs_live = require('fs');
const path_live = require('path');
// fixture не трекается в репо (.gitignore) — блок пропускается, если файла нет
const hasLiveFixture = fs_live.existsSync(path_live.join(__dirname, '..', '..', 'gemini-head-sample.txt'));
if (!hasLiveFixture) {
  console.info('gemini-head-sample.txt: фикстура отсутствует — пропуск');
}

(hasLiveFixture ? describe : describe.skip)('Gemini живой образец (gemini-head-sample.txt) через живой путь', () => {
  const fs = require('fs');
  const path = require('path');
  const logic = require('../../utils/gemini-intercept-logic');

  function loadLive() {
    const p = path.join(__dirname, '..', '..', 'gemini-head-sample.txt');
    const raw = fs.readFileSync(p, 'utf8');
    const r = parseGeminiHistory(raw);
    // эмуляция emitBaseSnapshot: orderItems из parsed messages (как turnsMap после одного ingest).
    const orderItems = r.messages.map((m, i) => ({
      id: m.id,
      turnId: String(m.id).replace(/_(user|assistant)$/, ''),
      r1: m.r1 || null,
  ...
  it('голова цепочки = user «Ты — Верховный Юридический Аудитор» → assistant «ЭКСПЕРТНОЕ ЗАКЛЮЧЕНИЕ…», inversions=0', () => {
    const { r, orderItems } = loadLive();

    const chain = logic.orderByR1Chain(orderItems);
    expect(chain.ok).toBe(true);

    const byId = {};
    for (const it of orderItems) byId[it.id] = it;
    const ids = chain.ids;
    const invItems = ids.map(id => byId[id]);
    expect(logic.countR1Inversions(invItems)).toBe(0);

    // голова: user + assistant одного головного хода, user первым.
    expect(byId[ids[0]].role).toBe('user');
    expect(byId[ids[1]].role).toBe('assistant');
    expect(byId[ids[0]].text).toContain('Ты — Верховный Юридический Аудитор');
    expect(byId[ids[1]].text).toContain('ЭКСПЕРТНОЕ ЗАКЛЮЧЕНИЕ');
  });

  it('user-блок головного хода не теряется и не смещается в середину', () => {
    const { orderItems } = loadLive();
    const verdictPos = orderItems.findIndex(m => /Верховный Юридический Аудитор/.test(m.text));
    expect(verdictPos).toBe(0); // самый первый элемент финального порядка
  });
});
