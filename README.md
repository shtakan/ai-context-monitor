...
Контекстное окно модели и реальная потеря контекста — не одно и то же. Модель начинает «забывать» ранние сообщения **раньше** формального лимита: при заполнении окна на небольшую часть (примерно 20–25%) модель уже может задать вопрос, ответ на который был в самом начале переписки.
